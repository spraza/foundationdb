#!/usr/bin/env bash
#
# setup.sh – spin up a two‑region FoundationDB cluster under fdbmonitor
#
# Required flags:
...
#   --stateless     4                          # stateless per main DC
...
usage() {
  cat <<EOF
Usage: $0 --fdb-monitor BIN --fdb-server BIN --base-dir DIR --port-start N \\
          --main-logs N --sat-logs N --main-stores N --stateless N \\
          --cluster-name NAME
EOF
  exit 1
}
...
# ---------- sanity checks ----------
vars=(FDB_MONITOR FDB_SERVER BASE_DIR PORT_START MAIN_LOGS SAT_LOGS MAIN_STORES STATELESS CLUSTER_NAME)
for v in "${vars[@]}"; do
...
until fdbcli -C "$CLUSTER" --exec 'status minimal' &>/dev/null; do sleep 1; done

fdbcli -C "$CLUSTER" --exec "coordinators auto"
fdbcli -C "$CLUSTER" --exec "configure new double ssd logs=$MAIN_LOGS usable_regions=2"
fdbcli -C "$CLUSTER" --exec "fileconfigure $REGIONS_JSON"
