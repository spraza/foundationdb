				...
				try {
					...
					if (watchValue != setValue) {
						if (watchValue.present())
							...
							tr->clear(setKey);
						TraceEvent("WatcherSetStart")
						    .detail("Watch", printable(watchKey))
						    .detail("Set", printable(setKey))
						    .detail("Value", printable(watchValue));
						wait(tr->commit());
						TraceEvent("WatcherSetFinish")
						    .detail("Watch", printable(watchKey))
						    .detail("Set", printable(setKey))
						    .detail("Value", printable(watchValue))
						    .detail("Ver", tr->getCommittedVersion());
					} else {
						TraceEvent("WatcherWatchStart")
						    .detail("WatchKey", printable(watchKey))
						    .detail("WatchValue", printable(watchValue))
						    .detail("SetValue", printable(setValue));
						state Future<Void> watchFuture = tr->watch(makeReference<Watch>(watchKey, watchValue));
						wait(tr->commit());
						TraceEvent("WatcherWatchCommitted")
						    .detail("Watch", printable(watchKey))
						    .detail("WatchValue", printable(watchValue));
						//.detail("SetValue", printable(setValue));
						if (BUGGIFY) {
	...
	Future<Void> watchesWorker(Database cx, WatchesWorkload* self) {
		...
		loop {
			...
			co_await cx.run([&](Transaction* tr) -> Future<Void> {
				...
				co_await tr->commit();
				TraceEvent("WatcherWorkerSetFinish")
				    .detail("Set", printable(startKey))
				    .detail("Value", printable(expectedValue))
				    .detail("Ver", tr->getCommittedVersion());
