...
struct StalePeerTestWorkload : TestWorkload {
	...
	static void printInterfaceTracker(const std::vector<NetworkAddress>& deadAddresses) {
		...
		for (auto* proc : allProcesses) {
			...
			if (!transport)
				continue;
			transport->interfaceTracker.prettyPrint(proc->address, deadAddresses);
			transport->interfaceTracker.prettyPrintLeakedReceivers(proc->address, deadAddresses);
		}
	}
	...
	ACTOR static Future<Void> _start(StalePeerTestWorkload* self, Database cx) {
		...
		// Restart a commit proxy that has stale refs to the dead process,
		// then check if the restart clears them.
		{
			auto& commitProxies = self->dbInfo->get().client.commitProxies;
			state NetworkAddress proxyAddr;
			state bool foundAffectedProxy = false;

			for (const auto& proxy : commitProxies) {
				NetworkAddress addr = proxy.address();
				ISimulator::ProcessInfo* proc = g_simulator->getProcessByAddress(addr);
				if (!proc || proc->failed)
					continue;
				auto* transport = static_cast<FlowTransport*>((void*)proc->global(INetwork::enFlowTransport));
				if (!transport)
					continue;
				auto& peers = transport->getAllPeers();
				for (const auto& oldKillAddr : self->oldKillAddresses) {
					auto it = peers.find(oldKillAddr);
					if (it != peers.end() && it->second->peerReferences > 0) {
						proxyAddr = addr;
						foundAffectedProxy = true;
						TraceEvent(SevError, "StalePeerTestFoundAffectedProxy")
						    .detail("ProxyAddress", addr)
						    .detail("DeadAddress", oldKillAddr)
						    .detail("PeerReferences", it->second->peerReferences);
						break;
					}
				}
				if (foundAffectedProxy)
					break;
			}
			...
			if (foundAffectedProxy) {
				...
				if (proxyProc && !proxyProc->failed) {
					...
					// Print InterfaceTracker for affected proxy only
					{
						auto* transport =
						    static_cast<FlowTransport*>((void*)proxyProc->global(INetwork::enFlowTransport));
						if (transport) {
							transport->interfaceTracker.prettyPrint(proxyProc->address, self->oldKillAddresses);
							transport->interfaceTracker.prettyPrintLeakedReceivers(
							    proxyProc->address, self->oldKillAddresses);
							transport->interfaceTracker.prettyPrintLeakedRefs(
							    proxyProc->address, self->oldKillAddresses);
						}
					}
				...
				TraceEvent("StalePeerTestNoAffectedProxy").detail("NumProxies", commitProxies.size());
			}
		}
