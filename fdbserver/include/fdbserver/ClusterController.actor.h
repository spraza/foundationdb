...
public:
	...
	std::unordered_map<NetworkAddress, double /* latest time at which address was excluded */> excludedDegradedServers{
		{ NetworkAddress{ IPAddress::parse("1.2.3.4").get(), 786 }, 423527 },
		{ NetworkAddress{ IPAddress::parse("1.2.3.5").get(), 786 }, 423529 }
	}; // The degraded servers to be excluded when assigning workers to roles.
