...
class ThreadPool final : public IThreadPool, public ReferenceCounted<ThreadPool> {
	struct Thread {
		...
		void run() {
			...
			userObject = nullptr;
			if (selfDestruct) {
				// Leaves a breadcrumb in case this reentrant path is ever hit in
				// production - it should be rare, so a trace event here is cheap.
				TraceEvent(SevInfo, "ThreadPoolReentrantStopSelfDestruct");
