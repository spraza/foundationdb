...
struct ArenaStatTypes {
	...
	/* ---------- readers ---------- */
	static KeyPtr stackSnapshot() noexcept; // lock-free immutable view

	/* ---------- writers ---------- */
	static void pushFrame(const std::string& label);
	static void popFrame();

private:
	static inline std::shared_ptr<const Key> g_stack_ = std::make_shared<Key>();
};
