...
void ServerKnobs::initialize(Randomize randomize, ClientKnobs* clientKnobs, IsSimulated isSimulated) {
	...
	*/
	...
	init( CC_ENABLE_REMOTE_TLOG_DEGRADATION_MONITORING,         true); if (isSimulated) CC_ENABLE_REMOTE_TLOG_DEGRADATION_MONITORING = deterministicRandom()->coinflip();
	init( CC_ENABLE_REMOTE_TLOG_DISCONNECT_MONITORING,          true); if (isSimulated) CC_ENABLE_REMOTE_TLOG_DISCONNECT_MONITORING = deterministicRandom()->coinflip();
	init( CC_ONLY_CONSIDER_INTRA_DC_LATENCY,                    true); if (isSimulated) CC_ONLY_CONSIDER_INTRA_DC_LATENCY = deterministicRandom()->coinflip();
	init( CC_INVALIDATE_EXCLUDED_PROCESSES,                     true); if (isSimulated) CC_INVALIDATE_EXCLUDED_PROCESSES = deterministicRandom()->coinflip();
	init( CC_GRAY_FAILURE_STATUS_JSON,                          true); if (isSimulated) CC_GRAY_FAILURE_STATUS_JSON = true;
	...
	init( GRAY_FAILURE_ALLOW_PRIMARY_SS_TO_COMPLAIN,           true ); if (isSimulated) GRAY_FAILURE_ALLOW_PRIMARY_SS_TO_COMPLAIN = deterministicRandom()->coinflip();
	init( GRAY_FAILURE_ALLOW_REMOTE_SS_TO_COMPLAIN,            true ); if (isSimulated) GRAY_FAILURE_ALLOW_REMOTE_SS_TO_COMPLAIN = deterministicRandom()->coinflip();
