...
public:
	...
	std::unordered_map<NetworkAddress, int> peerStreamCount;
...
ACTOR Future<Void> connectionKeeper(Reference<Peer> self,
	...
	loop {
		try {
			...
			if (self->peerReferences <= -1) {
				deletePeer = !self->transport->peerStreamCount.contains(self->destination);
			}
			if (self->transport->peerStreamCount.contains(self->destination)) {
				self->peerReferences += self->transport->peerStreamCount[self->destination];
				self->transport->peerStreamCount.erase(self->destination);
			}
			if (self->peerReferences == 0) {
				deletePeer = true;
			}
			if (!deletePeer) {
				deletePeer = now() - self->lastDataPacketReceivedTime > FLOW_KNOBS->STREAM_TRANSPORT_EXPIRY;
			}
