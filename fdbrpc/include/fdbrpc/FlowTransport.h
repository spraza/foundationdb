...
struct InterfaceTracker {
	struct Key {
		...
		bool operator==(const Key& other) const {
			return dstAddress == other.dstAddress && dstRole == other.dstRole;
		}
	};
	struct KeyHash {
	...
	void peerRefAdded(const NetworkAddress& addr) { peerRefCounts[addr].added++; }
	void peerRefRemovedRaw(const NetworkAddress& addr) { peerRefCounts[addr].removed++; }
	...
	// On destruction, the ID is marked as destroyed. At check time, any ID that
	// was created but not destroyed is a leaked FlowReceiver.
	...
	void flowReceiverDestroyed(const NetworkAddress& addr, int64_t id) {
		auto it = flowReceiverRecords.find(id);
		if (it != flowReceiverRecords.end()) {
	...
	int64_t promiseRefAdded(const NetworkAddress& addr, const UID& token) {
		...
		refRecords[id] =
		    RefRecord{ id, addr, token, g_network ? g_network->now() : 0.0, platform::get_backtrace(), false, false };
		return id;
	}
	...
	int64_t futureRefAdded(const NetworkAddress& addr, const UID& token) {
		int64_t id = nextRefId++;
		refRecords[id] =
		    RefRecord{ id, addr, token, g_network ? g_network->now() : 0.0, platform::get_backtrace(), false, true };
		return id;
	}
	...
	void prettyPrintLeakedRefs(const NetworkAddress& srcAddr,
	                           const std::vector<NetworkAddress>& filterAddrs) const {
